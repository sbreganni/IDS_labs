...
The main function for `rvest` is to access html pages:
...
Saying that we want paragraph 76 (absolute paths in general) can be dangerous because if anyone changes the website then you lose your content. It's better to use semantics
...
parsed_url |> 
  ...
  rvest::html_text()
#as long as the header id is still carnival we will get the right paragraph
...
```{r}
parsed_url |>
  rvest::html_element(xpath = '//div[contains(@class, "mw-heading mw-heading3")]//h3[@id="Roman_Cologne"]/following::p[2]') |>
  rvest::html_text()
...
spotify_table <- spotify_table_raw |>
  ...
  dplyr::slice(1:100) |> # drop last row (it's not a song)
...
spotify_table |>
  summarise(
    min_streams = min(streams_billions),
      )
...
We see that some songs are collaborations between artists, to consider them as well we can use regex:
...
setwd(tempwd) #not best practice!
```
...
for (i in seq_along(urls_list)) {
  ...
  if (!file.exists(paste0(folder, names_for_files[i]))) {
    ...
    Sys.sleep(runif(1, 0, 1)) #one random sleep interval of 0-1 seconds (anytime we do a forloop and scraping we pause R to make sure that the requests don't come in too fast and the server doesn't crash o_O)
  } 
}
