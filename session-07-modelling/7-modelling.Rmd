...
# a better way to do this is to create another variable with (x^2) inside and thne put that in the formula
```
...
```{r, echo= FALSE}
# a)
lm(life_expectancy ~ gdp + population + alcohol, data = life_expec_dat)
#or:
...
## lm() runs the whole regression but when I just call it I just get the coefficients. if I run asummary of the regression (either by saving it into an object or by putting summary() around the formula) then it gives me all the info I need.
...
# c)
life_expec_dat <- life_expec_dat |>
  dplyr::mutate(log_population = log(population),
                gdp_squared = gdp^2) #create log population variable
...
vars <- life_expec_dat |> 
  dplyr::select(-c("life_expectancy", "country")) |> #-c() means "everything but c"
  names()
...
```{r, echo= FALSE}
# from my understanding the object contains other objects within itself that each contain one regression

summary(multiv_model_out[[10]]) #percentage_expend. & alcohol
summary(multiv_model_out[[25]]) #alcohol & bmi & hiv_aids
summary(multiv_model_out[[31]]) #percentage_expend. & alcohol & bmi & hiv_aids
```
:::
...
```{r}
multiv_model_out_broom <- purrr::map_dfr(multiv_model_out[29:31], # model objects
                                         broom::tidy, # function to iterate with
...
```{r, echo= FALSE}
mod_table<-modelsummary::modelsummary(
  multiv_model_out[29:31], 
  output = "kableExtra",
  fmt = fmt_significant(2), # 2 signif. digit instead of fixed decimal places
  estimate  = "{estimate}",
  statistic = "conf.int",
  coef_omit = "Intercept",
  coef_rename=c("gdp"="Gdp", 
  ...
  row_spec(3, color = 'red') |>
  row_spec(5, background = 'pink') # changed based on your own preferences.
